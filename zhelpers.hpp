...
static std::string
s_recv (zmq::socket_t & socket,int flag_=0) {

    zmq::message_t message;
    try{
        socket.recv(&message,flag_);
    }catch(...){
    }
...
static bool
s_send (zmq::socket_t & socket, const std::string & string,int flag_=0) {
    bool rc=false;
    try{
    zmq::message_t message(string.size());
    memcpy (message.data(), string.data(), string.size());

    rc = socket.send (message,flag_);
    }
    catch(...){
    }
    return (rc);
...
static bool
s_sendmore (zmq::socket_t & socket, const std::string & string,int flag_=0) {
    ...
    try{
        ...
        rc = socket.send (message, ZMQ_SNDMORE|flag_);
    }
    catch(...){
    }
     return (rc);
